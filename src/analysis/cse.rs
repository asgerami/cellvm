...
pub fn analyze(f: &Function) -> CseReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        score += imms * 3 + span;
        if imms > 2 { notes.push(format!("cse: block {bi} imm-heavy")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    CseReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("cse score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
