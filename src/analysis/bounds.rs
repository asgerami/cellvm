//! static bounds hints for arrays.
...
pub fn analyze(f: &Function) -> BoundsReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        let windows = ir::windows(&ir).len() as i32;
        score += windows * 4 + span;
        if windows > 0 { notes.push(format!("bounds: windowed")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    BoundsReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("bounds score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
