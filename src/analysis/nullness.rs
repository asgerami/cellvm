...
pub struct NullnessReport {
    pub score: i32,
    pub notes: Vec<String>,
    pub hot_blocks: Vec<usize>,
}

pub fn analyze(f: &Function) -> NullnessReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        let calls = ir::count_calls(&ir) as i32;
        let throws = ir::count_throws(&ir) as i32;
        score += calls * 7 + throws * 11;
        if calls + throws > 0 { notes.push(format!("nullness: effectful")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    NullnessReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("nullness score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
...
pub fn explain(f: &crate::module::Function) -> String {
    let r = analyze(f);
    let mut s = format!("nullness total={}\n", r.score);
    for n in &r.notes {
        s.push_str("  - ");
        s.push_str(n);
        s.push('\n');
