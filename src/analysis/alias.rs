...
pub fn analyze(f: &Function) -> AliasReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        if locals > 4 { notes.push(format!("alias: wide frame")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    AliasReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("alias score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
