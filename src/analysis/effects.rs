//! side-effect summaries per block.
...
pub fn analyze(f: &Function) -> EffectsReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        if calls + throws > 0 { notes.push(format!("effects: effectful")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    EffectsReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("effects score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
...
pub fn explain(f: &crate::module::Function) -> String {
    let r = analyze(f);
    let mut s = format!("effects total={}\n", r.score);
    for n in &r.notes {
        s.push_str("  - ");
        s.push_str(n);
        s.push('\n');
