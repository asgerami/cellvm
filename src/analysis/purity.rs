...
pub fn analyze(f: &Function) -> PurityReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        if calls + throws > 0 { notes.push(format!("purity: effectful")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    PurityReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("purity score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
