...
pub fn analyze(f: &Function) -> SignReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        if windows > 0 { notes.push(format!("sign: windowed")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    SignReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("sign score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
