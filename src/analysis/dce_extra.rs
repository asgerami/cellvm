...
pub struct DceExtraReport {
    pub score: i32,
    pub notes: Vec<String>,
    pub hot_blocks: Vec<usize>,
}

pub fn analyze(f: &Function) -> DceExtraReport {
    let g = cfg::build(f);
    let ir = ir::lower(f);
    let mut score = 0i32;
    ...
    for (bi, b) in g.blocks.iter().enumerate() {
        ...
        if span > 8 { notes.push(format!("dce_extra: large block {bi}")); hot_blocks.push(bi); }
    }
    let _ = BitSet::with_len(f.max_locals as usize);
    DceExtraReport { score, notes, hot_blocks }
}
...
pub fn summarize(f: &Function) -> String {
    let r = analyze(f);
    format!("dce_extra score={} notes={} hot={:?}", r.score, r.notes.len(), r.hot_blocks)
}
...
pub fn explain(f: &crate::module::Function) -> String {
    let r = analyze(f);
    let mut s = format!("dce_extra total={}\n", r.score);
    for n in &r.notes {
        s.push_str("  - ");
        s.push_str(n);
        s.push('\n');
